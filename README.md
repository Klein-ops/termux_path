...
- 开机权限自动修复
  在开机 late_start 阶段自动修复 Termux 私有数据目录（/data/data/com.termux）的权限和 SELinux 上下文，确保命令可被普通用户执行。
...
5. 修复 Termux 目录权限
   执行 restorecon -R 恢复 SELinux 上下文，递归设置 /data/data/com.termux 目录权限为 755，临时目录权限为 1777（粘滞位），确保从外部调用时文件和目录的访问权限正常。
...
3. 判断该条目是否为本模块创建的 wrapper（通过检查软链接目标是否为 wrapper_main.sh，或文件内容包含模块标记）
...
1. 权限修改说明
   本模块会在开机时递归修改 Termux 私有数据目录（/data/data/com.termux）的权限为 755，并将其临时目录（tmp）权限设为 1777。这是为了确保从 Termux 外部调用其程序时，文件和目录的访问权限正常。未来版本可能会优化为仅修改必要的子目录。
...
3. 手动修复
   - 在 Root 终端执行：
     chmod -R 755 /data/data/com.termux/files/usr/bin/*
